...
2) \Rightarrow 1) \quad &\text{Jede Cauchy Folge ist beschränkt. Sei } \epsilon = 1 \Rightarrow \exists N \in N \sp \forall n, m \ge N \abs{a_n - a_m} < 1 \Rightarrow \abs{a_n - a_N} < 1 \\
...
&\text{Nach dem Satz von Bolzano-Weierstraß existiert eine konvergente Teilfolge }(a_{n_k})_{k \ge 0} \text{ sei } \liminf[k]{a_{n_k}} = a.\\
&\text{Wir zeigen. }\liminf{a_n} = a.\\
&\text{Sei }\epsilon > 0. \text{ Wähle $m$ so groß, dass }\abs{a_m - a_n} < \frac{\epsilon}{2} \forall n,m \ge N \text{ und } \abs{a_{n_k} - a} < \frac{\epsilon}{2} \sp \forall k \ge N\\

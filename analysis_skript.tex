...
\begin{tikzpicture}
        \draw[step=1,help lines,black!0] (-1,-1) grid (4,4);
        \draw[->] (-0.5,0) -- (3.5,0)  node[below right] {$\suminf{a_n}$};
        \draw[->] (0,-0.5) -- (0,3.5)  node[above left]{$\suminf{b_n}$};
        %\foreach \Point/\PointLabel in {(1,1), (1,2), (1,3), (1,4), (2,1)} % alle punkte 
% einzeichnungen bei x und y linien - und mit schrägenstrichen verbinden
\end{tikzpicture}
\end{center}
\end{satz}
...
\item f: \R_+ -> \R mit f(x) = \sqrt{x}
 g: \r-> \R
 f°g(x) = f(g(x)) = f(x^2) = \sqrt{x^2} = \abs{x}
\item
...
Sei $f : D \to \R$ eine Funktion, $a \in \R$ eine Zahl, sodass es mindestens eine Folge $(a_n)$ gibt mit $a_n \in D$ mit $\liminf{a_n} = a $ gibt. Man definiert \(\lim\limits_{x \to a}{f(x) = c\), wenn gilt: Für jede Folge (x_n)_{n\ge0} mit \liminf{x_n} = a  gilt lim\liminf{f(x_n)} = c.
c heißt dann Grenzwert
\end{defi}
 
\begin{defi}[Stetigkeit]
Sei f. D -> \R und a \in D . F heißt stetig in a, wenn  \(\lim\limits_{x \to a}{f(x) = f(a)\)
D.h. Für \jede Folge (x_n) mit lim(x_n) ) a ist lim(f(x_n)) = f(a)
...
\begin{bem}
f ist in a \in D nicht stetig \Leftrightarrow \exists  eine Folge (x_n) mit lim x_n = a aber die Folge (f(x_n) ) ist divergent oder
lim (f(x_n)) \ne f(a)
\end{bem}
...
\\
g°f : D \to \R. Sei x_n eine beliebige Folge mit \liminf{(x_n)} = a, da f stetig ist folgt: \lim{(f(x_n)} = f(a)
...
\[\forall \epsilon > 0 \exists \delta > 0 :  \forall x : \abs{x-a} < \delta \text{ gilt } \abs{f(x) - f(a)} < \epsilon\]
%bild: deltaepsilonkrit.png // stetige Funktion
%bild2: nichtdeltaepsilonkrit.png // Nicht stetige Funktion
\begin{proof}
\begin{enumerate}
\item[$\Leftarrow$]	Sei (x_n) eine Folge mit \lim{x_n} = a . zz lim(f(x_n)) ) f(a)
...
\item[$\Rightarrow$] Sei f in a stetig. zz ist das \epsilon-\delta-Kriterium.
...
Sei a\in \R und (x_n) eine Folge mit \limin{x_n} = a \Rightarrow \liminf{f(x_n)} = \liminf{x_n} = a = f(a)
...
Sei m \ge 1 \\
\abs{\sum_{n=1}^{m}{\frac{x^n}{n!}} \le \sum_{n=1}^{m}{\abs{\frac{x^n}{n!}} = \sum_{n=1}^{m}{\frac{\abs{x}^n}{n!} = \sum_{n= 0}^{m-1}{\frac{\abs{x}^{n+1}}{(n+1)!}} \le \sum_{n= 0}^{m}{\frac{\abs{x}^{n+1}}{(n+1)!}} = \abs{x}\sum_{n= 0}^{m}{\frac{\abs{x}^{n}}{(n+1)!}} \le \abs{x}\sum_{n= 0}^{m}{\frac{\abs{x}^{n}}{2^n}} = \abs{x}\sum_{n= 0}^{m}{\left(\frac{\abs{x}}{2}\right)^n}\\
Daraus folgt \abs{a^x - 1} = \abs{\sumInf{\frac{x^n}{n!}} = \liminf{\abs{\sum_{n=1}^{m}{\frac{x^n}{n!}}}} \le \liminf{\abs{x}\sum_{n=0}^{m}{\left(\frac{\abs{x}}{2}\right)^n}} = \abs{x}\suminf{\left(\underbrace{\frac{\abs{x}}{2}}_{< 1}\right)^n} = \abs{x}\frac{1}{1-\frac{\abs{x}}{2}} = \frac{2\abs{x}}{2-\abs{x}} \le 2\abs{x}
...
Sei a \in \R und (x_n) eine Folge mit \limin{x_n} = a \Rightarrow \liminf{x_n - a} = 0. Da exp stetig in 0, folgt \liminf{exp(x_n -a) = exp(0) = 1 
\Rightarrow \liminf{exp(x_n)} = \liminf{e^{x_n}} = \liminf{e^{(x_n - a)+a} \overset{Funktionsgleichung}{=} \liminf{e^{x_n-a}e^a = e^a \underbrace{\liminf{e^{x_n-a}}}_{=1} = e^a ^= e^a.
...
Funktionen:\\
$ f : \R \to \R \text{ mit } x \mapsto f(x) $ \\
$ f : \R \times \R \to \R \text{ mit } (a,b) \mapsto f(a,b) $\\
$p(x) = \sum_{k=0}{n}{a_kx^k}  $\\
